...
class _JobsSectionState extends State<JobsSection> {
  ...
  Future<void> _loadUserSkillsAndJobs() async {
    ...
    try {
      ...
      final myPostedJobsList = myJobsSnap.docs.map((doc) => {
            'id': doc.id,
            ...doc.data(),
          }).toList();
...
class JobCard extends StatelessWidget {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                job['description']?.substring(0, job['description'].length > 120 ? 120 : null) ?? '',
              ...
              if (skills.length > 5)
                Padding(
                  ...
                  child: Text("+${skills.length - 5} more",
  ...
  void _showJobDetailsDialog(BuildContext context) {
    ...
    final applicants = (job['applicants'] as List<dynamic>?)?.cast<String>() ?? [];
  ...
  Future<void> _showApplicantsList(BuildContext context) async {
    ...
    try {
      final applicantsUids = (job['applicants'] as List<dynamic>?)?.cast<String>() ?? [];
      final requiredSkills = (job['requiredSkills'] as List<dynamic>?)?.cast<String>()
      ...
      for (String appUid in applicantsUids) {
        // Fetch user profile
        final userDoc = await FirebaseFirestore.instance.collection('users').doc(appUid).get();
        ...
        if (skillsDoc.exists) {
          ...
          final topSkills = (skillsData['top_skills'] as List<dynamic>?)?.cast<String>() ?? [];
          final otherSkills = (skillsData['other_skills'] as List<dynamic>?)?.cast<String>() ?? [];
          final industrySkills = (skillsData['industry_skills'] as List<dynamic>?)?.cast<String>() ?? [];
        ...
        final matchCount = appSkills.where((skill) => requiredSkills.contains(skill)).length;
      ...
      applicantProfiles.sort((a, b) => b['matchCount'].compareTo(a['matchCount']));
                              ...
                              children: [
                                ...
                                if (matchingSkills.isNotEmpty)
                                  Wrap(
                                    spacing: 6,
                                    runSpacing: 6,
                                    children: matchingSkills.map((skill) => Chip(
                                          label: Text(skill),
                                          backgroundColor: Colors.green.shade100,
                                        )).toList(),
                                  )
                                else
                                  const Text('No matching skills', style: TextStyle(color: Colors.grey)),
                              ],
                            ),
                          ),
                        );
