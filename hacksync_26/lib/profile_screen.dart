...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      final bytesSnap = await _firestore.collection('users').doc(uid).collection('bytes').orderBy('createdAt', descending: true).get();

      final bundlesSnap = await _firestore.collection('users').doc(uid).collection('bundles').orderBy('createdAt', descending: true).get();
      ...
      setState(() {
        ...
        userBytes = bytesSnap.docs.map((doc) => {'id': doc.id, ...doc.data()}).toList();
        userBundles = bundlesSnap.docs.map((doc) => {'id': doc.id, ...doc.data()}).toList();
  ...
  Future<void> _addOrEditExperience(Map<String, dynamic>? exp) async {
                ...
                children: [
                  Expanded(
                    child: OutlinedButton(
                      ...
                      child: Text(
                        startDate == null ? "Start Date" : "Start: ${startDate!.year}-${startDate!.month.toString().padLeft(2, '0')}",
                      ),
                    ),
                  ),
                  ...
                  Expanded(
                    child: OutlinedButton(
                      ...
                      child: Text(
                        endDate == null ? "End Date (or Present)" : "End: ${endDate!.year}-${endDate!.month.toString().padLeft(2, '0')}",
                      ),
                    ),
                  ),
                ],
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(isEdit ? "Experience updated!" : "Experience added!")),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
      ...
      slivers: [
        ...
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: const Text("Your Bytes", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          ),
        ),
        SliverList(
          delegate: SliverChildBuilderDelegate(
            (context, index) {
              ...
              return ListTile(
                ...
                trailing: Text(
                  byte['createdAt'] != null
                      ? (byte['createdAt'] as Timestamp).toDate().toString().split(' ')[0]
                      : '',
                ),
              );
            },
        ...
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: const Text("Your Bundles", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          ),
        ),
        SliverPadding(
                ...
                return GestureDetector(
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => BundleDetailScreen(bundleId: bundle['id']),
                      ),
                    );
                  },
                  child: Card(
            ...
            child: Row(
              ...
              children: [
                ...
                IconButton(
                  ...
                  onPressed: () => _editField(title: "About", initialValue: _about, fieldName: 'about', maxLines: 5),
                ),
              ],
            ),
          ),
